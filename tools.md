...
- Studio 文稿树当前正式口径：只保留少量固定语义槽位 `设定 / 大纲 / 正文`，其中 DB 真值文稿 `大纲/总大纲.md`、`大纲/开篇设计.md`、`正文/第xxx章.md` 继续只读；默认模板里的 `项目说明`、设定细分文稿、`数据层/*.json`、章节规划、时间轴、附录、校验、导出等项目文稿会在新建项目时写入文件层，其中 `项目说明.md` 由当前 `ProjectSetting` 生成初稿，其他模板默认空白，之后允许用户增删改。`数据层` JSON 仅用于结构化资料，不替代 `ProjectSetting` 或正文主真值。`正文` 现在允许新增卷目录，并通过章节路径占位文件把 DB 章节挂到对应目录下；这些占位文件不作为第二套正文真值。节点改名/删除时，会同步重映射聊天已选上下文路径；若影响当前打开文稿，还会同步更新 `doc` 路由。
- 项目摘要当前正式口径：`project_setting` 仍是机器摘要真值；`项目说明.md` 是人类编辑的简版主说明。项目设置页“从项目说明提取”会把 `项目说明.md` 作为输入来源，但会携带当前 `project_setting` 作为 base 做增量合并，避免简版说明把未提及的结构化字段冲掉。
