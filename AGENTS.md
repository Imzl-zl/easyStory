...
## 6. Agent 协作文件规则
- 开始任务前，若项目根存在 `tools.md` / `memory.md`，则先读；只读取当前项目根协作文件，不引入额外全局目录协议，也不无目的批量加载。
- `tools.md` 只记录可复用的项目协作知识、命令、模式和坑点；允许整理重写，但写入前必须先读。
- `memory.md` 是**当前状态快照 + 最近活跃窗口**，整体覆盖更新而不是追加；目标 ≤100 行，硬限制 ≤120 行，只保留当前基线、已完成能力、进行中方向、关键决策、仍需注意的坑点，以及最近 2~3 天的一句话级活跃窗口。
- 过程细节不进 `memory.md`；代码变更历史看 git log，协作 / 调试 / 根因叙事进 `memory/archive/`。
- `memory/archive/` 按月追加重要协作历史，供调试回溯时按需读取，不在每次会话加载。
- 写入时机：完成一个完整功能 / 修复闭环后再写，不是每个小改动都写；会话结束前如有未写入的重要变更，也应补写。
- 更新 `memory.md` 时：刷新当前基线和最后更新日期；新增闭环能力；移除已完成的进行中项；仅保留仍有效的关键决策和坑点；活跃窗口只保留最近 2~3 天，超出的删除，超长时优先压缩活跃窗口和已完成能力措辞。
- 更新 `memory/archive/` 时：仅记录重要根因分析、调试洞察和踩坑；格式保持 `## [日期 | 标题]` + `Events / Changes / Insights`；不记录 Validation 命令快照。
- 反复验证的稳定规律应整理进 `tools.md` 或正式文档，不留在 `memory.md` 或归档中重复。
- `tools.md` / `memory.md` 属于协作辅助，不是产品功能真值；发生冲突时，以正式设计、配置和当前代码为准，它们只负责帮助恢复上下文，不负责改写正式规则。
