...
export function EngineTaskPanel({ projectId, workflow }: EngineTaskPanelProps) {
  ...
  const regenerateMutation = useMutation({
    mutationFn: () => regenerateChapterTasks(projectId, toRegeneratePayload(draftRows)),
    onSuccess: (result) => {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.02fr_0.98fr]">
      <section className="panel-muted space-y-4 p-5">
        ...
        {tasksQuery.isLoading ? <p className="text-sm text-[var(--text-secondary)]">正在加载章节任务...</p> : null}
        {tasksQuery.error ? (
        ...
        {tasks.length === 0 && !tasksQuery.isLoading ? (
          ...
          <div className="space-y-3">
            {tasks.map((task) => {
              ...
              return (
                <article key={task.task_id} className="rounded-2xl border border-[rgba(101,92,82,0.12)] bg-[rgba(255,255,255,0.56)] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{task.brief}</p>
      ...
      <div className="space-y-4">
        ...
        <EngineTaskRegenerateSection
          ...
          onAddRow={() =>
            setDraftRows((current) => [...current, buildEmptyDraftRow(nextChapterNumber(current))])
          }
