...
import type { ProjectSettingImpactSummary } from "@/lib/api/types";

type ProjectSettingImpactPanelProps = {
...
export function ProjectSettingImpactPanel({ impact }: ProjectSettingImpactPanelProps) {
  return (
    <section className="panel-muted space-y-4 rounded-[28px] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          <p className="text-sm text-[var(--text-secondary)]">最近一次保存影响</p>
          <p className="text-sm leading-6 text-[var(--text-secondary)]">
            {impact.has_impact
              ? "本次设定变更已经同步到下游真值，以下内容已被标记为 stale。"
              : "本次保存没有触发新的 stale 传播，当前下游内容保持原状。"}
          </p>
        </div>
        <StatusBadge
