...
import { formatTemplateTime } from "@/features/lobby/components/template-library-support";
...
export function TemplateLibraryDetailPanel({ model }: { model: TemplateLibraryModel }) {
  if (model.detailQuery.isLoading) {
    return <section className="panel-shell p-5 text-sm text-[var(--text-secondary)]">正在加载模板详情...</section>;
  }
  ...
  return (
    <section className="panel-shell fan-panel space-y-5 p-5">
      ...
      <div className="space-y-2">
        <h3 className="font-serif text-lg font-semibold">核心配置</h3>
    ...
    <header className="space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <StatusBadge
          ...
          label={template.is_builtin ? "官方朱砂印" : "自定义"}
        />
      </div>
      <div className="flex flex-wrap gap-2">
        <button className="ink-button-secondary" onClick={model.startDuplicate} type="button">基于此创建副本</button>
        {!template.is_builtin ? <button className="ink-button-secondary" onClick={model.startEdit} type="button">编辑模板</button> : null}
  ...
  return (
    <dl className="grid gap-3 md:grid-cols-2">
      {[
        ...
        ["workflow_id", template.workflow_id ?? "未配置"],
        ["引导问题", String(template.guided_questions.length)],
        ["节点数量", String(template.nodes.length)],
...
function GuidedQuestionSection({ template }: { template: TemplateDetail }) {
  if (template.guided_questions.length === 0) {
    return <p className="rounded-2xl bg-[rgba(255,255,255,0.52)] px-4 py-3 text-sm leading-6 text-[var(--text-secondary)]">当前模板没有引导问题，可直接作为空白法度使用。</p>;
  }
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-3">
        {template.guided_questions.map((question) => (
          <article key={question.variable} className="panel-muted space-y-1 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--text-secondary)]">{question.variable}</p>
...
function NodeSection({ template }: { template: TemplateDetail }) {
  return (
    <div className="space-y-3">
      <h3 className="font-serif text-lg font-semibold">节点快照</h3>
      <div className="space-y-3">
          ...
          <article key={node.id} className="panel-muted space-y-3 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-sm leading-6 text-[var(--text-secondary)]">{node.node_type} · skill: {node.skill_id || "未绑定"}</p>
              </div>
