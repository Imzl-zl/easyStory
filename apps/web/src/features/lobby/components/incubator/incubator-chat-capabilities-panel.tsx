...
export function ChatCapabilitiesPanel({
  ...
  return (
    <>
      <section className="rounded-2xl bg-muted shadow-sm px-3 py-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="text-[12px] font-medium text-text-primary">先从这里开始</p>
        ...
        <p className="mt-3 rounded-2xl bg-glass px-3 py-2 text-[11px] leading-5 text-text-secondary">
          如果你只是想直接聊天，保持这里的默认值就可以。
        </p>
        {hasSelectedAgent ? (
          <p className="mt-3 rounded-2xl bg-accent-soft px-3 py-2 text-[11px] leading-5 text-accent-primary">
            你已经在“高级能力”里切换了 Agent，它会优先使用自己绑定的 Skill。
          </p>
        ) : null}
        {skillQueryError ? (
          <p className="mt-3 rounded-2xl bg-accent-danger/10 px-3 py-2 text-[11px] leading-5 text-accent-danger">
            {getErrorMessage(skillQueryError)}
          </p>
        ) : null}
      </section>

      <details className="overflow-hidden rounded-2xl bg-glass shadow-glass">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 text-[12px] font-medium text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-primary/15 focus-visible:ring-inset">
          <span>更多能力</span>
          <Tag
            bordered={false}
            className="!m-0 !rounded-full !bg-glass-heavy !px-2.5 !py-1 !text-[10.5px] !leading-4 !text-text-secondary"
            size="small"
          >
        ...
        <div className="space-y-3 border-t border-line-soft px-3 py-3">
          ...
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="rounded-2xl bg-muted shadow-sm px-3 py-3">
              <p className="text-[12px] font-medium text-text-primary">自动动作</p>
              <p className="mt-1 text-[11px] leading-5 text-text-secondary">
          ...
          {agentQueryError ? (
            <p className="rounded-2xl bg-accent-danger/10 px-3 py-2 text-[11px] leading-5 text-accent-danger">
...
function SettingsLink({
  ...
  return (
    <Link
      className="inline-flex items-center justify-center rounded-full bg-surface shadow-xs px-3 py-1.5 text-[12px] font-medium text-accent-primary transition hover:bg-accent-soft hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-primary/15 focus-visible:ring-offset-2 focus-visible:ring-offset-[rgba(255,255,255,0.74)]"
      href={href}
    >
      {children}
    </Link>
