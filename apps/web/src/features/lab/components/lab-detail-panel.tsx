...
function LabDetailContent({
  ...
  if (isLoading && !analysis) {
    return <p className="text-sm text-text-secondary">正在加载洞察详情...</p>;
  }
...
function LabAnalysisMeta({ analysis }: Readonly<{ analysis: AnalysisDetail }>) {
  return (
    <div className="space-y-3 rounded-2xl bg-muted shadow-sm p-4">
      ...
      <p className="text-sm leading-6 text-text-secondary">来源标题：{analysis.source_title ?? "未命名来源"}</p>
      <p className="text-sm leading-6 text-text-secondary">关联正文片段：{analysis.content_id ?? "未绑定内容"}</p>
      <p className="text-sm leading-6 text-text-secondary">创建时间：{formatLabAnalysisTime(analysis.created_at)}</p>
    </div>
  );
}
