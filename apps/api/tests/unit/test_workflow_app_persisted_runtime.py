...
async def test_workflow_app_persisted_runtime_runs_success_path() -> None:
    ...
    runtime = WorkflowAppPersistedRuntime(
        ...
        run_runtime=lambda loaded_workflow: _return_async(
            call_log.append(("run", loaded_workflow))
        ),
...
@pytest.mark.asyncio
async def test_workflow_app_persisted_runtime_recovers_business_rule_failure_without_error_reason() -> (
    None
):
    call_log: list[object] = []
...
@pytest.mark.asyncio
async def test_workflow_app_persisted_runtime_does_not_reuse_previous_current_node_id_when_reload_fails() -> (
    None
):
    call_log: list[object] = []
    workflow = SimpleNamespace(current_node_id="chapter_gen")
    ...
    runtime = WorkflowAppPersistedRuntime(
        ...
        run_runtime=lambda loaded_workflow: _return_async(None),
        commit=lambda: _return_async(None),
        recover_runtime_failure=lambda current_node_id, detail, reason: _return_async(
            call_log.append((current_node_id, detail, reason))
        ),
    ...
    assert call_log == [(None, "load failed", "error")]
