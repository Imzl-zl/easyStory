...
def _validate_write_execution_context(context: AssistantToolExecutionContext) -> None:
    ...
    base_version = _read_active_buffer_base_version(context)
    grant_base_version = approval_grant.base_version_constraints.get(active_document_ref)
    if grant_base_version != base_version:
        raise AssistantToolTerminalRunError(
            "write_grant_expired",
...
def _read_active_buffer_base_version(context: AssistantToolExecutionContext) -> str | None:
    if not isinstance(context.active_buffer_state, dict):
        return None
    value = context.active_buffer_state.get("base_version")
