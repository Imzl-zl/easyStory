...
def _has_grant_bound_write_access(context: AssistantToolExposureContext) -> bool:
    ...
    if active_buffer_state.get("dirty") is not False:
        return False
    base_version = active_buffer_state.get("base_version")
    return isinstance(base_version, str) and bool(base_version.strip())
...
def _build_turn_approval_grant(
    *,
    ...
    base_version = str(context.active_buffer_state["base_version"])
    ...
    return AssistantToolApprovalGrant(
        ...
        base_version_constraints={target_document_ref: base_version},
