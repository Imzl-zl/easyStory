...
## [2026-03-24 | 协作文件边界收口]
- **Events**：修正最近几轮协作文件治理中过度流程化的问题，把记忆/工具文件收回到原本用途。
- **Changes**：`AGENTS.md` 已收回为项目级硬规则；`tools.md` 只保留跨会话稳定知识；`memory.md` 重新压回纯状态快照，不再承载恢复流程说明；`docs/` 维持正式设计真值，但只按需查。
- **Insights**：这套协作文件的目标是帮助模型更快读写和维护项目上下文，不是再造一套固定流程；规则、稳定知识、当前快照、历史根因四层分清即可。
