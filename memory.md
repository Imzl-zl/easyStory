...
- 2026-04-07：assistant runtime context governance 与规则治理主线已收口：项目范围工具提示现已由 prepare 阶段先解析 internal discovery decision，再按本轮 visible tools 收口后投影成 `tool_guidance_snapshot`，进入 `AssistantTurnContext / AssistantTurnRunSnapshot / AssistantTurnRun`，并由 prompt projection / prompt render 与 `NormalizedInputItem(item_type=tool_guidance)` 共同消费，且当前 snapshot 已显式冻结 `discovery_source`；`document_context` 当前已分成 `document_context_snapshot / document_context_bindings_snapshot / document_context_recovery_snapshot / document_context_injection_snapshot` 四层运行时真值，其中 latest recovery view 仍会进入 `NormalizedInputItem(item_type=document_context_recovery)`，而 prompt projection / prompt render / run snapshot / hook payload 共用同一份 `document_context_injection_snapshot`；initial prompt compaction 与 continuation compaction 当前都已显式冻结前后投影视图 digest，并把 `soft / hard / fail` 收口为共享 helper/终止错误；规则 runtime bundle 当前已支持主 `AGENTS.md` frontmatter `include` 的同作用域递归展开，include 顺序稳定，循环 include / 缺失文件 / 越出作用域根目录都会显式报错，且主文件正文更新时会保留已有 include frontmatter；assistant 目录标准化现已形成 `service/turn/`、`service/tooling/`、`service/context/`、`service/hooks_runtime/`、`service/rules/`、`service/preferences/`、`service/skills/`、`service/agents/`、`service/mcp/` 与 `service/hooks/` 十个稳定子域，且 prompt render 已从根级 `assistant_execution_support.py` 收回 `service/context/assistant_prompt_render_support.py`；主计划当前已推进到 `9 DONE / 0 IN_PROGRESS / 0 TODO`，相关计划文档已降级为历史实施记录，统一 assistant 后端回归已通过 `155 passed`。
