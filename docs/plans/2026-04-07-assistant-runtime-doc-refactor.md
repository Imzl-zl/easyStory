...
>
> 本文记录 2026-04-07 这轮 assistant runtime 文档重构的实施过程与收口结果，现已降级为历史实施记录，不再作为当前迁移入口。
>
...
本计划当时的核心任务，是先把“怎么改文档”讲清楚，再按章节逐步回写正式设计。
...
- first-class tool discovery 仍未进入完整代码真值，暂不应在正式设计里冒充现状；当前计划范围内的 compaction / context governance / rule include 已进入 runtime 合同
...
- 收尾阶段的逐步路线图见 [2026-04-07-assistant-runtime-remaining-roadmap](./2026-04-07-assistant-runtime-remaining-roadmap.md)
...
- initial prompt compaction snapshot 当前已同时记录 `compressed_messages_digest + projected_messages_digest + summary_anchor_keywords + protected_document_paths + protected_document_refs + protected_document_reasons + protected_document_binding_versions + document_context_collapsed + document_context_projection_mode + projected_document_context_snapshot + document_context_recovery_snapshot`，且 `projected_document_context_snapshot` 已和显式 `document_context_injection_snapshot` 对齐；`NormalizedInputItem(item_type=compacted_context)` 的 `payload` 也已直接复用完整 snapshot，不再把 `summary` 拆成第二套派生真值
...
- latest tool loop continuation budget compaction 已进入显式 `continuation_compaction_snapshot`，并会记录 `compressed_items_digest + projected_items_digest + compacted_tool_names + compacted_document_refs + compacted_document_versions + compacted_catalog_versions`
- `document_context` 已通过 normalized binding 进入 run snapshot、grant 与 project capability；latest `document_context_recovery_snapshot` 已进入显式 run 真值，并同步进入 `NormalizedInputItem(item_type=document_context_recovery)`；latest `document_context_injection_snapshot` 也已进入 `AssistantTurnContext / AssistantTurnRunSnapshot / AssistantTurnRun` 与 hook payload，prompt projection / prompt render 复用同一份 injection view
- 项目范围工具提示已同时进入 prompt、`NormalizedInputItem(item_type=tool_guidance)` 与显式 `tool_guidance_snapshot`；当前由 prepare 阶段先解析 internal discovery decision，再按本轮 visible tools 收口后投影成 guidance snapshot，复用于 `AssistantTurnContext / AssistantTurnRunSnapshot / AssistantTurnRun`、prompt projection 与 prompt render，不再在 prompt 渲染阶段二次重算；当前 guidance snapshot 也会显式冻结 `discovery_source`
- `before_assistant_response / after_assistant_response` hook payload 当前也会同步暴露 latest `request.document_context_bindings_snapshot`、`request.document_context_recovery_snapshot`、`request.document_context_injection_snapshot`、`request.compaction_snapshot`、`request.tool_guidance_snapshot`、`request.tool_catalog_version` 与 `request.exposed_tool_names_snapshot`，让 hook payload 与 turn snapshot 共用同一份 context / tool exposure 真值
- 当前计划范围内的分级 compaction / collapse / recovery 已落地为正式代码真值；若后续需要多次 continuation compaction 的完整历史链，再另起计划，不再挂在本轮未完成项中
...
- 当前规则仍以每个作用域一份主 `AGENTS.md` 作为设置页读写真值
- runtime `rule bundle` 已支持主文件 frontmatter `include` 的同作用域递归展开，且装配顺序稳定
- 循环 include、缺失文件、越出当前作用域根目录的 include 当前都会显式报错
- `/api/v1/assistant/rules/*` 更新主文件正文时，当前会保留已有 `include` frontmatter，不把 include 配置静默抹掉
...
3. 之后进入统一审查与主计划收口；流式预执行继续只保留为后续评估项

更细的逐步执行顺序已单独整理到 [2026-04-07-assistant-runtime-remaining-roadmap](./2026-04-07-assistant-runtime-remaining-roadmap.md)。
...
| `document_context` 单真值收口 | `20 + 21 + 22` | DONE | request projection、normalized binding、project binding、latest `document_context_recovery_snapshot` 与 latest `document_context_injection_snapshot` 当前已形成同一条 runtime 真值链，并同步进入 run snapshot、normalized input、hook payload 与 compaction audit；initial prompt `compaction_snapshot / compacted_context` 也已显式冻结 `compressed_messages_digest + projected_messages_digest + protected_document_reasons + summary_anchor_keywords + document_context_collapsed + document_context_projection_mode + projected_document_context_snapshot + document_context_recovery_snapshot`，当前范围内的 collapse / 审计语义已收口完成 |
| ordinary chat 启发式工具提示收口 | `20` | DONE | 当前已收口为“prepare 阶段先解析 internal discovery decision，再按本轮实际 visible tools 收口后投影成 `tool_guidance` snapshot，并由 `AssistantTurnContext / AssistantTurnRunSnapshot / AssistantTurnRun`、prompt projection / prompt render 与 `NormalizedInputItem(item_type=tool_guidance)` 共同消费”；当前 snapshot 也已显式冻结 `discovery_source`，后续 first-class discovery 另行评估，不再阻塞本轮收口 |
| 分级 compaction / collapse / 恢复 | `20 + 22` | DONE | 当前已落地 turn 入口历史摘要压缩、`phase / level` 元信息、latest `continuation_request_snapshot`、tool loop continuation projection 的预算收口，以及 latest `continuation_compaction_snapshot` 审计；initial prompt 与 continuation 两侧当前都已补齐前后投影视图 digest，并把 `soft / hard / fail` 语义显式收口为 helper/共享终止错误：`soft=仅内部 trimming`，`hard=消息/item 骨架删除或槽位删除`，`fail=共享 budget_exhausted terminal error`；当前 latest snapshot 合同已稳定，多次 compaction 历史链不再作为本轮阻塞项 |
| 规则拆分 / include | `20` | DONE | 当前规则仍以主 `AGENTS.md` 作为设置页真值，但 runtime `rule bundle` 已支持 frontmatter `include` 的同作用域递归展开；include 顺序稳定，循环 include / 缺失文件 / 越出作用域根目录都会显式报错，且主文件更新时会保留已有 include frontmatter |
...
- 主计划已收口为 `9 DONE / 0 IN_PROGRESS / 0 TODO`
- `20 / 21 / 22` 已吸收对应主归属主题
- 本文仅保留为历史实施记录，不再作为当前迁移入口
