...
- `stale` 表示上游 `ProjectSetting` / `Outline` / `OpeningPlan` 已变化，当前章节计划已过期，不得继续注入到 `chapter_gen`
...
恢复时：
- 若当前 `workflow_execution_id` 下存在 `stale` 的 `chapter_tasks`，先阻止恢复并要求重新执行 `chapter_split`
- 否则找到第一个 `status` 不在 `("completed", "skipped")` 的章节（如 pending/interrupted/failed）→ 从该章节继续
...
章节生成（Skill 调用）
  ...
  │         ├─ 审核通过 → 章节完成
  │         ├─ reviewer 执行失败 → 视为审核阶段执行失败，不触发内容精修
  │         └─ 审核失败 → 触发精修
  │                       ├─ 精修成功 → 重新审核
