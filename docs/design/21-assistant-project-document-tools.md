...
- `binding_version`
  - 标识某个 `path -> document_ref + writable/source/source_kind` 绑定视图的稳定版本
...
| 工具 | 作用 | 一期优先级 |
|---|---|---|
| `project.list_documents` | 列出 assistant 可访问的统一文稿目录 | `v1B` |
| `project.search_documents` | 文稿候选发现；`v1B` 先按路径名 / 标题 / 基础元数据检索 | `v1B` |
...
- `unsupported_write_target_scope`
...
- `story.data_schema.v2`
- `story.characters.v1`
- `story.factions.v1`
- `story.character_relations.v1`
- `story.faction_relations.v1`
- `story.memberships.v1`
- `story.events.v1`
...
- `run_audit_id`
  - 指向本轮 turn 级 append-only 工具审计记录
