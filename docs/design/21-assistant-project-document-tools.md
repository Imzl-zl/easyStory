...
| `project.edit_document` | 对单份项目文稿执行确定性局部文本替换 | `v1B` |
...
`paths` 是正文内容读取入口，不是候选发现入口；单次调用最多读取 8 份文稿。需要更多候选时先用 `project.search_documents` 缩小范围，再分批读取。
...
### 8.3 `project.write_document` 默认整份写回，不做 patch 式编辑
...
### 8.3A `project.edit_document` 精确局部编辑

`project.edit_document` 是独立工具，不改变 `project.write_document` 的全量语义。
在同一活动文稿和同一基线下，它与 `project.write_document` 共享同一个 `grant_bound` 写入授权。
...
- `base_version`
- `edits[]`
  - `old_text`：必填，要替换的原文片段
  - `new_text`：必填，替换后的文本；允许为空字符串表示删除
  - `context_before`：可选，紧邻 `old_text` 之前的原文锚点
  - `context_after`：可选，紧邻 `old_text` 之后的原文锚点

执行规则：

- 所有编辑先在同一份原文上完成定位校验，再原子应用
- `old_text + context_before + context_after` 必须唯一定位一处目标
- 命中 0 处返回 `edit_target_not_found`
- 命中多处返回 `edit_target_ambiguous`
- 多个编辑目标重叠返回 `edit_target_overlaps`
- 不做 fuzzy match、近似文本猜测、行号漂移修复或隐式 fallback
- 应用后的下一版完整内容继续走 `prepare_write_document / commit_prepared_write_document`，复用 `base_version / buffer_hash / source / binding_version / revision / schema_id` 校验
