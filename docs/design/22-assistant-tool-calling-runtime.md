...
当前 ordinary chat 仍是单次请求-响应链：

- `AssistantTurnRequestDTO / AssistantTurnResponseDTO` 仍是“消息输入 + 文本输出”模型
- `AssistantService` 当前执行链是 `_prepare_turn -> _call_turn_llm -> _finalize_turn`
- `ToolProvider` 当前实质更接近 LLM 调用器
- Hook / MCP / Plugin 路径只服务 hook 事件，不是 ordinary chat 的正式工具循环
- existing `before / after / on_error` hooks 仍围绕“单次 LLM 请求”触发，还没有正式 run / step 生命周期协议
- SSE 当前只有 `chunk / completed / error`
- Studio 仍把当前文稿内容、当前路径、附加路径拼进用户消息文本，不是正式结构化运行时输入
- 当前 active conversation 主要还停留在前端本地状态，request contract 里没有正式 `conversation_id`

因此当前 assistant runtime 还没有：

- 通用 tool loop
- turn 级 run 真值
- step 级 tool 真值
- provider continuation state
- 结构化 `document_context`
- 显式幂等键、预算快照与事件去重锚点
...
进入原生 tool-calling runtime 后，应显式拆开两层：

- `shared/runtime`
  - `AssistantModelClient`
  - `AssistantModelRequestAdapter`
  - `AssistantModelResponseNormalizer`
  - `AssistantModelToolCapabilities`
- `assistant/service`
  - `AssistantService`
  - `AssistantToolLoop`
...
这样 workflow / incubator / verifier 仍能复用同一套 provider transport 与方言语义，不会反向复制 assistant 私有实现。
...
Entry(SSE/HTTP)
  -> AssistantService
    ...
    -> AssistantToolExposurePolicy
    -> AssistantToolLoop
      -> AssistantModelClient(shared/runtime)
      -> AssistantToolExecutor
...
- `AssistantToolDescriptorRegistry`
  - 维护工具正式合同真值
- `AssistantToolExposurePolicy`
  - 根据当前 turn context、授权、预算、目录状态、模型能力决定本轮实际暴露哪些工具
- `AssistantToolExecutor`
...
- `continuation_anchor_snapshot`
  ...
  - `v1A` 最少应能落到 `previous_run_id` 与可选 `messages_digest`
...
- `compaction_snapshot`
  - 保存本轮是否发生 context compaction、压缩了哪些 section，以及压缩产物摘要
...
- `v1A` 实际运行只开放 `approval_state in {not_required, approved}`
...
- `v1A` 即使暂不对模型暴露 `always_confirm`，run store / SSE / resume 协议也应先为该对象留正式位置
...
- `PromptSection[]`
  - 面向装配、预算、compaction 与 cache hint
- `NormalizedInputItem[]`
  - 面向 run snapshot、continuation 与 provider replay

`PromptSection` 不是长期恢复真值；任何需要被恢复、审批、审计复用的系统语义，都必须进入 `NormalizedInputItem[]`。
...
- `document_context`
  - 保存归一化后的 `DocumentContextBinding[]` 或等价引用
- `tool_call`
...
- `NormalizedInputItem[]` 才是 replay 真值；`PromptSection[]` 只是当前请求的装配投影
...
- `AssistantModelRequestAdapter`
  - 把统一 runtime 语义映射到具体 provider 请求
- `AssistantModelResponseNormalizer`
  - 把 provider 响应折回统一 item 语义
...
进入原生 tool-calling runtime 后，建议口径直接定为：

- `AssistantModelClient`
  - 成为 ordinary chat 主链的正式模型调用入口
- `LLMToolProvider`
  - 仅保留为迁移期兼容层或非 ordinary chat 旧链路适配层
- `ToolProvider`
  - 不再继续承担 assistant 主 runtime 的长期抽象中心

这样可以避免后续在 `shared/runtime` 内长期并存两套职责重叠的“模型调用抽象”。

### 9.2.2 `PromptSection[]` 到 provider 请求的投影

provider 适配层不应再把 assistant 输入长期视为一个大字符串 prompt。

建议长期输入模型固定为有序 `PromptSection[]`：

- runtime 负责 section 归一化、预算裁剪与 compaction
- request adapter 负责把 `PromptSection[]` 投影成具体 provider 需要的 `system / developer / messages / input items`
- 当前代码里的 `system_prompt + prompt` 只是迁移期 projection，不再反向定义 runtime 语义
- run store 持久化与 continuation 真值应落在 `NormalizedInputItem[]`，而不是直接把 `PromptSection[]` 当恢复快照

这样可以保证：

- provider 角色差异不污染 assistant 主模型
- prompt cache / section priority / compaction 可以由 runtime 统一治理
- 后续即使 provider 改 role 语义，也不需要回头改 ordinary chat 主骨架
...
1. `return_error_to_model`
   - 适用于读取失败、参数校验失败、只读工具超时、可解释的 provider/tool schema 错误
   - runtime 把错误封装成失败的 `tool_result envelope` 回填给模型
   - 由模型决定是否换路径、缩小范围、重新读取或结束回答
2. `terminal_run_failure`
   - 适用于权限拒绝、写入 grant 失效、dirty buffer 阻塞、预算耗尽、恢复协议缺失、显式取消后不可继续
...
- 当总输入逼近 `max_input_tokens`、`max_history_tokens` 或工具结果预算上限时，runtime 可以触发 compaction
- compaction 只作用于旧历史、旧工具结果摘要和低优先级 section
- 当前用户消息、当前 `document_context`、当前 grant 约束、待审批/待提交工具结果不得被静默压掉
- 带 `truncated=true + next_cursor` 的读取结果，在模型尚未消费前应视为高优先级“待继续读取”结果，不作为首轮 compaction 目标

输出原则：

- compaction 结果应回写为显式的 `PromptSection(section_kind=compacted_context)` 或等价结构化片段
- `AssistantTurnRun.compaction_snapshot` 必须记录触发原因、被压缩 section 与压缩结果摘要
- compaction 改的是 runtime 输入视图，不改 `messages` transcript 真值，也不改持久化会话历史
- 若 compaction 后仍超限，应显式返回 `budget_exhausted`，而不是继续偷偷裁剪
...
- `apps/api/app/modules/assistant/service/`
  ...
  - `assistant_tool_policy.py`
  - `assistant_tool_exposure_policy.py`
  - `assistant_tool_executor.py`
  - `assistant_run_store.py`
  - `assistant_tool_step_store.py`
- `apps/api/app/shared/runtime/`
  - `assistant_model_client.py`
  - `assistant_model_request_adapter.py`
  - `assistant_model_response_normalizer.py`
  - `assistant_model_capabilities.py`
...
3. 请求输入正式收口到 `conversation_id + continuation_anchor + messages + document_context + requested_write_scope + requested_write_targets + input_data(兼容袋)`
...
16. 请求装配内部收口为 `PromptSection[]`；run snapshot / continuation 真值收口为 `NormalizedInputItem[]`
...
| turn 输入 | 正式收口到 `conversation_id + continuation_anchor + messages + document_context + requested_write_scope + requested_write_targets + input_data(兼容袋)` |
...
| Prompt 真值 | `PromptSection[]` 只负责装配/预算/compaction；恢复与审批真值落在 `NormalizedInputItem[]` |
...
18. 活动文稿缺失可信 `active_buffer_state` 时，不得对其签发写入 grant；非 active 目标若没有独立 buffer 快照，也不得在 `v1A` 获得写入 grant
...
22. `PromptSection` 不能作为恢复与审批唯一真值；需要 replay 的系统语义必须进入 `NormalizedInputItem[]`
