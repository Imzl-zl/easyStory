...
| `deleted_at != null` | 回收站项目 | 允许恢复、彻底删除与手动清空回收站 |
### 4.2 设定完整度检查
...
- 恢复：恢复成功后，回收站列表即时刷新，并展示顶部反馈。
- 彻底删除：必须二次确认，明确提示会同步清理关联内容、工作流、导出、事实、账单、审计和项目级凭证。
- 清空回收站：作为页头危险操作，必须二次确认，不根据搜索过滤结果做局部清空。
