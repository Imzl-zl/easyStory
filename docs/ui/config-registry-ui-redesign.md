...
│  │                                                      │   │
│  │  节点类型过滤 (可选)                                  │   │
│  │  ☑ draft_generation    ☑ review    ☐ export        │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  执行条件 (可选)                                             │
│  ────────────                                                │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  字段        [chapter.word_count         ]          │   │
│  │  操作符      [>=                      ▼]             │   │
│  │  值          [1000                        ]          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
...
- `config` 为 `dict[str, Any]`，根据 `action_type` 显示不同的表单字段
- `input_mapping` 路径辅助：显示可用变量路径（如 `{{workflow.node.id}}`）
