...
详细世界观、人物小传、伏笔和长期约束，长期仍应回到项目文档本身维护。当前 `Studio` 已补齐项目文稿文件读写，文件默认落在 `apps/api/.runtime/project-documents/projects/<project_id>/documents/`，便于直接备份、复制和迁移；但这层文件目前只承载 `设定/*`、`附录/*`、`大纲/章节规划.md` 等工作文稿，不替代 `contents + content_versions` 的正式正文真值，正式 `大纲 / 开篇设计 / 正文章节` 仍走内容主链。
...
>
> 其中 assistant 规则使用 `AGENTS.md`，AI 偏好使用 `preferences.yaml`，Skills 使用 `skills/<skill_id>/SKILL.md`，MCP 使用 `mcp_servers/<server_id>/MCP.yaml`，用户个人 Agent 使用 `agents/<agent_id>/AGENT.md`。当前普通用户主路径已经收口为“规则文件 + Skills + MCP”，运行时统一按“项目层 -> 用户层 -> 系统层”解析；AI 偏好继续使用字段级覆盖，`Agents / Hooks` 保留为高级能力。Web 设置页本质上是在编辑这些文件，而不是把规则正文当数据库主真值。
