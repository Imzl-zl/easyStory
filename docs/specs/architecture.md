...
easyStory 是一个 **AI 小说创作平台**，核心目标不是再做一个通用 agent 工作台，而是把小说创作做成可恢复、可审查、可持续迭代的状态化系统。

核心特点：

1. 以创作工作流和结构化真值为中心，而不是以通用 agent 面板为中心
2. 允许通过规则、Skill、Agent、Hook、MCP 增强创作能力，但这些能力只服务小说业务
3. 把模型调用、工具调用、审查、回退、版本留存在同一条产品主链中
4. 为未来知识库接入预留边界，但不把知识检索提前写成新的业务真值源

可以借鉴 Cursor / Claude Code 这类产品在 rules、tool、MCP、hook 上的交互经验，但 easyStory 的产品主心智仍然是“设定、大纲、章节、审查、修订、连续性”。
...
Agent 是"带运行时策略的角色封装"，用于把某个 Skill、模型偏好、MCP/Hook 能力和输出约束打包成一个显式入口。Agent 是高级增强，不是普通聊天默认入口。
...
LangGraph / LiteLLM 以及后续可能引入的 LangChain 都属于实现层，不属于 easyStory 的业务真值层。

- LangGraph 负责状态化流程、暂停恢复、人工确认等通用编排
- LangChain 当前仍是预留实现候选；若后续接入，则负责高层 agent 便利层、tool/middleware/MCP adapter 等通用能力
- LiteLLM 负责 southbound 模型网关；极少数显式不适合 LiteLLM 的连接场景走 native HTTP backend
- 真正的业务真值仍由 `project / content / review / context / workflow` 模块持有

因此，框架对象不得直接充当以下真值：

- 项目文稿 identity / revision
- 正文版本链
- 规则文件与 Skill 装配结果
- 审查结论
- 知识库检索结果的正式归属
...
┌─────────────────────────────────────────────────────────────────┐
│                        用户界面层 (UI Layer)                     │
│  项目大厅 | 创作工作台 | Assistant 对话 | 规则/模型设置           │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                        平台核心层 (Platform Core)                │
│  项目管理 | 内容管理 | 审查管理 | 上下文管理 | 导出管理          │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                        创作引擎层 (Engine Layer)                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │ LangGraph    │  │ Skill/规则   │  │ 预留 Agent   │          │
│  │ 流程编排     │  │ 装配         │  │ 便利层候选   │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │ Tool runtime │  │ Hook 系统    │  │ MCP 集成     │          │
│  └──────────────┘  └──────────────┘  └──────────────┘          │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                        配置系统层 (Config Layer)                 │
│  Rules 配置 | Skills 配置 | Agents 配置 | Hooks/Workflow 配置   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                        能力接入层 (Capability Layer)             │
│  LiteLLM backend | native backend | 外部 MCP | 未来知识检索后端  │
└─────────────────────────────────────────────────────────────────┘
```
...
| 平台核心层 | 项目管理、内容管理、上下文与审查编排 | Python、FastAPI |
| 创作引擎层 | 工作流执行、Skill/规则装配、tool runtime、预留 agent 便利层 | LangGraph、ToolProvider、PluginRegistry |
| 配置系统层 | 规则、Skill、Agent、Hook、Workflow 配置加载与校验 | YAML、文件监听 |
| 能力接入层 | 模型网关、外部工具接入、未来知识检索后端 | LiteLLM、native_http backend、MCP、向量/全文检索后端 |
...
| 向量数据库/知识库 | v0.1 暂不引入；后续作为 `context` 模块能力扩展接入 | 不改变正文、文稿和审查的主真值归属 |
...
│  LangGraph orchestration | ToolProvider | PluginRegistry        │
│  PluginRegistry                                                │
└─────────────────────────────────────────────────────────────────┘
          │
          ▼
...
│  Database | LiteLLM backend / native_http backend | FileSystem  │
│  ConfigLoader | future knowledge retrieval backends            │
└─────────────────────────────────────────────────────────────────┘
```
