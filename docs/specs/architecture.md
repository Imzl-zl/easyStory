...
LangGraph / LangChain / LiteLLM 都属于实现层，不属于 easyStory 的业务真值层。

- LangGraph 负责状态化流程、暂停恢复、人工确认等通用编排
- LangChain 负责高层 agent 便利层、tool/middleware/MCP adapter 等通用能力
...
| 创作引擎层 | 工作流执行、Skill/规则装配、tool runtime、agent 便利层 | LangGraph、LangChain、ToolProvider、PluginRegistry |
...
│  LangGraph orchestration | LangChain helpers | ToolProvider     │
│  PluginRegistry                                                │
└─────────────────────────────────────────────────────────────────┘
          │
          ▼
